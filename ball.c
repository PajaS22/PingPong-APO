...
void update_ball(Position pos, Position old_pos, int radius, int old_radius, pixel color, pixel *frame_buff) {
    draw_ball(old_pos, old_radius, BACKGROUND_COLOR, frame_buff);  // deleting the old ball from the frame buffer
    draw_ball(pos, radius, color, frame_buff);                     // drawing a new ball
}
